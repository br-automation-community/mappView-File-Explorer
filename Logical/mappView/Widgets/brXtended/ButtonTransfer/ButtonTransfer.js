...
define([
     ...
     * @requires widgets.brease.FileExplorerSystem
     * 
    ...
    var defaultSettings = {
        ...
        WidgetClass = widgets.brease.Button.extend(function ButtonTransfer() {
            widgets.brease.Button.apply(this, arguments);
        }, defaultSettings),
    ...
    p.init = function () {
        ...
        if (!config.editMode) {
            ...
            if (!config.preLoadingState) {
                // FileExplorer handling
                this.fileExplorer = new widgets.brease.FileExplorerSystem();
                // Init com to FileHandlerComm 
                this.fileManager = widgets.brease.FileManager.createWidget(this.elem.id);
            }
        }

        widgets.brease.Button.prototype.init.call(this);
    };
    ...
    */
    ...
    p._saveFile = function _saveFile(path, flags, encoding, data) {
        if (config.preLoadingState) return;
    ...
    p._errorHandling = function _errorHandling(code) {
        console.log("Transfer Widget Error:" + code);
        ...
        * @iatStudioExposed
        * @param {Number} response Number of error transmitted by the mapp component.
        */
    ...
    p.dispose = function () {
        var that = this;
        widgets.brXtended.ButtonTransfer.prototype.dispose.apply(that, arguments);
    };
