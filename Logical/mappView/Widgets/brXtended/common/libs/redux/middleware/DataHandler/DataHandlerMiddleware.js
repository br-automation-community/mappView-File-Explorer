...
define([
    'brease',
    'widgets/brXtended/common/libs/redux/reducers/DataHandler/DataHandlerActions'
], function ({ callWidget,
    config: breaseConfig,
    events: { BreaseEvent },
    enum: { Enum },
    ...
    /**
     * @class widgets.brXtended.common.libs.redux.middleware.DataHandler.DataHandlerMiddleware
    ...
    MiddleWare._initChildren = function (widget, store) {
        ...
        widget.el.find('[data-brease-widget]').each(function () {
            ...
            if (uiController.getWidgetState(id) >= Enum.WidgetState.INITIALIZED && uiController.getWidgetState(id) !== Enum.WidgetState.SUSPENDED) {
                d.resolve();
            } else {
    ...
    MiddleWare.childrenInitializedHandler = function (widget, childrenIdList, store) {
        ...
        childrenIdList.forEach(function (id) {
            var childrenWidget = callWidget(id, 'widget');
    ...
    MiddleWare._setChildWidgetEnableState = function (widget) {
        widget.settings.childrenIdList.forEach(function (id) {
            uiController.callWidget(id, 'setParentEnableState', !widget.isDisabled);
        });
    };
